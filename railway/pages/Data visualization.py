...
chart = alt.Chart(df2).mark_bar().encode(
    x=alt.X('type', axis=alt.Axis(labelAngle=90)),   # Set the x-axis label and rotate it 90 degrees
    y='distance',                                    # Set the y-axis label
    color=alt.Color('type', legend=None)             # Set the color to differentiate between bars
).properties(
    width=800,                                       # Set the width in pixels
    height=400                                       # Set the height in pixels
)
...
st.subheader('100% of GR trains have third AC, 100% of Mail trains have sleeper and 95% of JShtb have chair_car')

df2 = df[["type","first_class","first_ac","second_ac","third_ac","sleeper","chair_car"]].groupby("type").mean()
...
colors = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd"]  # define your colors here
