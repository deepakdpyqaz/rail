...
def extract_station_data(train_no,data):
    ...
    if data["error"]!="":
        ...
        for station_data in data["response"]["station_data"]:
            for station in station_data["values"]:
                ...
                extracted_station_data["longitude"].append(
    ...
    station_order = CategoricalDtype(categories=station_data.code.tolist(), ordered=True)        
...
if train_name:
    with st.spinner("Fetching realtime data..."):
        ...
        station_data,df,original_delays,current_station = extract_station_data(train_no,data)
        X = df[
            [
                "third_ac",
                "zone",
    ...
    station_name_order = CategoricalDtype(station_data.name.tolist(), ordered=True)
    if current_station:
        st.markdown(
        ...
        org_df = pd.DataFrame(
            {
                "Station Name": station_data.name.tolist()[: current_station_idx + 1],
                "Delay": list(original_delays.values())[: current_station_idx + 1],
            }
        )
        org_df = org_df.assign(source="Original")
        expected_df = pd.DataFrame(
            {"Station Name": station_data.name.tolist(), "Delay": expected_delays}
        )
        ...
        expected_df = pd.DataFrame({"Station Name": station_data.name.tolist(), "Delay": expected_delays})
        new_df = expected_df.assign(source="Predicted")
    
    st.subheader("Expected Delays (in minutes)")
